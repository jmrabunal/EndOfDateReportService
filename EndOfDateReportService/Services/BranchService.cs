...
namespace EndOfDateReportService.Services
{
    public class BranchService : IBranchInterface
    {
        ...
        public async Task<IEnumerable<Branch>> GenerateReport(DateTime startDate, DateTime endDate)
        {
            if(!await _repository.TryGetReport(startDate))
            {
                ...
                foreach (var branch in branches)
                {
                   ...
                   var section =  _configuration.GetSection("LanesByBranch").GetSection(branch.Id.ToString());
                   var lanes  = section.AsEnumerable();
                   foreach (var laneKey in lanes)
                   {
                       int.TryParse(laneKey.Value, out int lane);
        ...
        public void PdfGenerator(DateTime date, int branchId)
        {
            ...
            if (branch != null)
            {
                _pdfService.GenerateBranchPaymentMethodsPdf(branch);
            }
        }
    }
};
